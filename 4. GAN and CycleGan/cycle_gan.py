...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = torch.sum(torch.pow(D_X(images_X)-1, 2)) / opts.batch_size
        D_Y_loss = torch.sum(torch.pow(D_Y(images_Y)-1, 2)) / opts.batch_size
        ...
        D_X_loss = torch.sum(torch.pow(D_X(fake_X), 2)) / opts.batch_size

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = torch.sum(torch.pow(D_Y(fake_Y), 2)) / opts.batch_size
        ...
        if opts.use_cycle_consistency_loss:
            ...
            cycle_consistency_loss = torch.sum(torch.pow(images_Y - reconstructed_Y,2))/opts.batch_size
